  ...
  $.fn.edslider = function (settings) {
    ...
    this.each(function () {
      ...
      if (options.position == 0 || options.position > sliderLi.length) {
        console.error(
          "error: start position value must be between 1 and " +
            sliderLi.length +
            "!"
        );
        startPosition = 1;
      ...
      if ((options.navigator || options.paginator) && sliderLi.length > 1) {
        ...
        if (options.paginator) {
          ...
          paginatorLi = paginator.find("li").on("mouseenter", function () {
            if (interact) {
              position = $(this).index();
              if (index - 1 != position) {
                sliderLi
        ...
        if (options.navigator) {
          controls
            .append(
              '<div class="navigator prev"/><div class="navigator next"/>'
            )
            .find(".navigator")
            .hide()
            .on("click", function () {
      ...
      function pause() {
        ...
        timeLeft =
          (progressWidth - progressElapsed) *
          (options.interval / progressWidth);
      }
